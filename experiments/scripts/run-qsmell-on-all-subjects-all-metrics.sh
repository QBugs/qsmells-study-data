...
parallel --version > /dev/null 2>&1 || die "[ERROR] Could not find GNU 'parallel' to run jobs in parallel. Please install GNU 'parallel' and re-run the script."
...
jobs_file="/tmp/$USER-$$-$(echo $RANDOM | md5sum | cut -f1 -d' ')-jobs.txt"
rm -f "$jobs_file"; touch "$jobs_file"
...
while read -r row; do
  ...
  for smell_metric in "CG" "ROC" "LC" "IM" "IdQ" "IQ"; do
    ...
    echo "bash \"$SCRIPT_DIR/run-qsmell.sh\" \
            --input_file_path \"$matrix_file_path\" \
            --smell_metric \"$smell_metric\" \
            --output_file_path \"$output_file_path\" > \"$job_log_file_path\" 2>&1" >> "$jobs_file"
  done
...
# Shuffle jobs
shuf "$jobs_file" > "$jobs_file.tmp" && mv "$jobs_file.tmp" "$jobs_file"

# Run jobs
parallel --progress \
  -j $(cat /proc/cpuinfo | grep 'cpu cores' | sort -u | cut -f2 -d':' | cut -f2 -d' ') \
  -a "$jobs_file" || die "[ERROR] Failed to run jobs in parallel!"
